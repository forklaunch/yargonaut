...
function Yargonaut() {
  ...
  function upToFirstColon(str) {
    ...
    return {
      render: str.substring(0, firstColon),
  ...
  function applyFont(font, keys, force) {
    ...
    keys.forEach(function (k) {
      ...
      else if (force && yargsKeys[k].transform === null)
        yargsKeys[k].transform = wholeString;
    });
  ...
  function applyStyleWorkaround(keys) {
    ...
    keys.forEach(function (k) {
      ...
      if (fontWorkaround[k]) {
        if (typeof fontWorkaround[k] === "string")
          u[k] = doStyle(k, fontWorkaround[k], styles[k]);
        else {
          u[k] = {
            one: doStyle(k, fontWorkaround[k].one, styles[k]),
            other: doStyle(
              yargsKeys[k].plural,
              fontWorkaround[k].other,
              styles[k]
            ),
          };
        }
  ...
  function applyFontWorkaround(keys) {
    ...
    keys.forEach(function (k) {
      ...
      if (styleWorkaround[k]) {
        if (typeof styleWorkaround[k] === "string")
          u[k] = doRender(k, styleWorkaround[k], fonts[k]);
        else {
          u[k] = {
            one: doRender(k, styleWorkaround[k].one, fonts[k]),
            other: doRender(
              yargsKeys[k].plural,
              styleWorkaround[k].other,
              fonts[k]
            ),
          };
        }
  ...
  function hack() {
    ...
    require.cache[found].exports = function (opts) {
      ...
      function tweak(key, origString, newString, figlet, font) {
        ...
        return ocdf
          ? ocdf(key, origString, newString, figlet, font)
          : newString;
      }
  ...
  function findInModuleCache(lookingFor) {
    ...
    if (require.cache) {
      for (
        var i = 0, files = Object.keys(require.cache);
        i < files.length;
        i++
      ) {
